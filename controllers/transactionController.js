 ...
 */
const processOrderPayment = asyncHandler(
  async (orderId, totalAmount, commissionData, paymentMethod = "card") => {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        const transactionId = `ORD_${Date.now()}_${MakeID(16)}`;
 ...
 * @returns {Object} - Transaction record
 */
const processRefund = asyncHandler(
  async (orderId, refundAmount, reason = "Customer request") => {
    const session = await mongoose.startSession();
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        const transactionId = `REF_${Date.now()}_${MakeID(16)}`;
        ...
        if (dispatchRefund > 0 && order.deliveryAgent) {
          ...
          if (dispatchWallet) {
            await dispatchWallet.deductFunds(dispatchRefund, "refund", session);
          }
        }
      });
...
const reverseTransaction = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    try {
      await session.withTransaction(async () => {
        reversedTransaction = await transaction.reverse(reason, _id, session);
      });
    } finally {
      await session.endSession();
    }
    ...
    audit.log({
      ...
      metadata: { reason, reversedId: reversedTransaction.transactionId },
    });
  } catch (error) {
    throw new Error(error.message);
  }
