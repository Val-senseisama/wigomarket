...
vatConfigSchema.methods.getVATResponsibility = function (
  vendorData,
  transactionAmount,
) {
  ...
  if (
    this.responsibility.platform.threshold &&
...
// Post-save hook to audit VAT changes
vatConfigSchema.post("save", function (doc) {
  const audit = require("../services/auditService");
  audit.log({
    action: "config.vat_updated",
    actor: { userId: doc.metadata?.lastUpdatedBy, role: "system" },
    resource: { type: "vatConfig", id: doc._id },
    metadata: { version: doc.metadata?.version, status: doc.status },
  });
});
